...
export class CustomerEffects {
  getCustomer$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatMap((action) => of(action).pipe(withLatestFrom(this.store.select(fromCustomer.selectContext)))),
  ...
  getById$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatMap((action) => of(action).pipe(withLatestFrom(this.store.select(fromCustomer.selectAll)))),
  ...
  loadById$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action) => this.http.get<Customer>(`${this.baseUrl}/${action.id}`)),
  ...
  addCustomer$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatMap(({ customer, redirectSupplier }) =>
        this.http.post<{ id: number; customers: Customer[] }>(this.baseUrl, customer).pipe(
          map(({ id }) => ({
  ...
  updateCustomer$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatMap(({ customer, redirect }) =>
        this.http.put<Customer>(this.baseUrl, customer).pipe(map((updatedCustomer) => ({ customer: updatedCustomer, redirect })))
      ),
  ...
  removeCustomer$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatMap(({ customer, redirect }) =>
        this.http.delete<void>(`${this.baseUrl}/${customer.id}`).pipe(map(() => ({ customer, redirect })))
      ),
  ...
  constructor(private actions$: Actions, private http: HttpClient, private router: Router, private store: Store<CustomerAppState>) {}
}
