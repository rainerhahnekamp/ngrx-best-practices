...
export class EditContainerComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private store: Store<CustomerAppState>) {}

  ngOnInit(): void {
    const id = Number(this.route.snapshot.params.id);
    this.customer$ = this.store.select(fromCustomer.selectById, id).pipe(map((customer) => ({ ...customer })));
  }
